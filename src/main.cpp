/* Original song code here:
https://github.com/robsoncouto/arduino-songs/blob/master/silentnight/silentnight.ino
Edited to use non-blocking Neotimer instead of delay.
It also blinks a led. */
...
int thisNote = 0;
Neotimer soundTimer = Neotimer();

Neotimer ledTimer = Neotimer();
int ledLight = 0;
int ledUp = false;
...
void loop() {
  ...
  if ((soundTimer.done()) || (thisNote == 0)) {
    if (thisNote < notes * 2) {
      ...
      soundTimer = Neotimer(noteDuration);
      soundTimer.start();
      thisNote = thisNote + 2;
    } else {
      thisNote = 0;
